...
def update_productline(db: Session, productline_id: str, productline: schemas.ProductLineCreate):
    db_productline = db.query(models.ProductLine).filter(
        models.ProductLine.productLine == productline_id).first()
...
def update_product(db: Session, product_code: str, product: schemas.ProductCreate):
    db_product = db.query(models.Product).filter(
        models.Product.productCode == product_code).first()
...
def update_office(db: Session, office_code: str, office: schemas.OfficeCreate):
    db_office = db.query(models.Office).filter(
        models.Office.officeCode == office_code).first()
...
def update_employee(db: Session, employee_number: int, employee: schemas.EmployeeCreate):
    db_employee = db.query(models.Employee).filter(
        models.Employee.employeeNumber == employee_number).first()
...
def update_customer(db: Session, customer_number: int, customer: schemas.CustomerCreate):
    db_customer = db.query(models.Customer).filter(
        models.Customer.customerNumber == customer_number).first()
...
def update_payment(db: Session, customer_number: int, check_number: str, payment: schemas.PaymentCreate):
    db_payment = db.query(models.Payment).filter(models.Payment.customerNumber ==
                                                 customer_number, models.Payment.checkNumber == check_number).first()
...
def update_order(db: Session, order_number: int, order: schemas.OrderCreate):
    db_order = db.query(models.Order).filter(
        models.Order.orderNumber == order_number).first()
...
def update_orderdetail(db: Session, order_number: int, product_code: str, orderdetail: schemas.OrderDetailCreate):
    db_orderdetail = db.query(models.OrderDetail).filter(
        models.OrderDetail.orderNumber == order_number, models.OrderDetail.productCode == product_code).first()
